function fish_user_key_bindings
	...
	bind \cg fzf_cur_dir_widget		# 아래 f 와 연동하여 g로 지정했지만 alt-c 의 단축키를 변경한 거
	bind \cf fzf_full_dir_widget	# 'f'ull 의 의미로 씁니다. $HOME부터의 절대경로를 표시합니다
    ...
    if bind -M insert > /dev/null 2>&1
		...
		bind -M insert \cg fzf_cur_dir_widget			# 
		bind -M insert \cf fzf_full_dir_widget
