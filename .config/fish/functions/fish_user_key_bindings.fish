function fish_user_key_bindings
	fzf_key_bindings
	bind \cr fzf-file-utylee-widget		# 원래대로 파일명을 검색하는 명령어입니다 (현재디렉토리부터)
	bind \cf fzf-dir-utylee-widget		# 아래 f 와 연동하여 g로 지정했지만 alt-c 의 단축키를 변경한 거
    ...
    if bind -M insert > /dev/null 2>&1
		bind -M insert \cr fzf-file-utylee-widget
		bind -M insert \cf fzf-dir-utylee-widget		# 
